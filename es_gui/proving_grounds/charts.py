...
class BarChart(Chart):
    ...
    def generate_bars(self, bar_data):
        ...
        if max_value > 0:
            max_value_scale = 1
            if max_value >= 1000000 and max_value<1000000000:
                self.y_axis_format += 'M'
                max_value_scale = 1000000
            if max_value >= 1000000000:
                self.y_axis_format += 'B'
                max_value_scale = 1000000000
            max_label = Label(pos=(0.5*x0 - self.width/2, self.max_height + self.y_padding - self.height/2),
                            text=self.y_axis_format.format(max_value/max_value_scale), halign='right', color=[0, 0, 0, 1])
            self.add_widget(max_label)
