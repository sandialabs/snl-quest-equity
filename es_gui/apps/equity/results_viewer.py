...
class PowerPlantResultsViewer(EquityResultsViewer):
    ...
    def draw_figure(self, *args):
        ...
        if plot_type == 'plant dispatch':
            ...
            ax.set_xlim(-130,-60)
            ax.set_ylim(20,55)
            ax.plot(self.power_plant_data["lon"],self.power_plant_data["lat"],"o",color='gold',markersize=1)
            ax.text(self.power_plant_data["lon"],self.power_plant_data["lat"]-0.05*35,self.power_plant_data["Name"],size=6,color='gold',horizontalalignment='center', verticalalignment='center')
