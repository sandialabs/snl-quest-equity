...
class PeakerRepReportScreen(ReportScreen):
    ...
    def generate_cost_bar_chart(self, *args):
        ...
        self.title.text = "Estimated costs for replacing the selected power plant."
    ...
    def generate_power_energy_bar_chart(self, *args):
        ...
        self.title.text = "Estimated pv and battery sizes needed to replace the selected power plant."
        self.desc.text = "\n "
        report_templates = ''
        for ix, op in enumerate(self.chart_data, start=0):
            label = op[0]
    ...
    def generate_pollution_bar_chart(self, *args):
        ...
        self.title.text = "Estimated health benefits of from reduced pollution."
        ...
        text = ('Total Health benefits : Low Estimate {0}, High Estimate {1}'.format(format_long_dollar_string(eo.Pollution_Low_Value),format_long_dollar_string(eo.Pollution_High_Value))+'\n'
    ...
    def generate_disadvantaged_stackedbar_chart(self, *args):
        ...
        self.title.text = "Estimated fraction of health benefits that would go to disadvantaged communities."
        self.desc.text += ' '.join(report_templates)
    ...
    def generate_low_income_stackedbar_chart(self, *args):
        ...
        # generate report text
        self.title.text = "Estimated fraction of health benefits that would go to people with low incomes."
        self.desc.text += ' '.join(report_templates)
...
class PeakerRepGenerateReportMenu(ModalView):
    ...
    def __init__(self, **kwargs):
        ...
        self.open_report_button.disabled = True
    ...
    def generate_executive_summary(self,eo,name):
        ...
        executive_summary_string.append(header_text)
        ...
        executive_summary_string.append(sizing_text)

        cost_text = ('  - The PV would cost approximately {0} and the ESS would cost approximately {1}, making the total capital cost estimate {2}.'.format(
        ...
        executive_summary_string.append(cost_text)
        ...
        executive_summary_string.append(health_text)
        ...
        executive_summary_string.append(scc_text)
        ...
        executive_summary_string.append(sNPV_text)
        ...
        executive_summary_string.append(disad_text)
        ...
        executive_summary_string.append(low_income_text)

        return executive_summary_string
    ...
    def generate_report_from_template(self):
        ...
        # build three tables for results Investment, Impact, and Equity
        investment_results_table = []
        impact_results_table = [] 
        equity_results_table = [] 

        # Build the top row for Investment, Impact tables
        replacement_fractions_ROW = [] 
        for ix, op in enumerate(self.host_report.chart_data, start=0):
            eo = op[1]
            replacement_fractions_ROW.append(eo.replacement_fraction*100)

        investment_results_table.append(replacement_fractions_ROW)
        impact_results_table.append(replacement_fractions_ROW)
        equity_results_table.append(replacement_fractions_ROW)

        # Build all rows
        investment_results_table_row_PV_power = [] 
        investment_results_table_row_ESS_power = [] 
        investment_results_table_row_ESS_energy = [] 
        investment_results_table_row_PV_cost = [] 
        investment_results_table_row_ESS_cost = [] 
        investment_results_table_row_total_cost = [] 

        impact_results_table_row_hb_low = [] 
        impact_results_table_row_hb_high = [] 
        impact_results_table_row_co2_off = [] 
        impact_results_table_row_climate_ben = [] 
        impact_results_table_row_snpv_health_low = [] 
        impact_results_table_row_snpv_health_high = [] 
        impact_results_table_row_snpv_climate = [] 
        impact_results_table_row_snpv_total_low = [] 
        impact_results_table_row_snpv_total_high = [] 

        equity_results_table_row_dac_pop = []
        equity_results_table_row_li_pop = []
        equity_results_table_row_dac_ben = []
        equity_results_table_row_dac_snpv_low = []
        equity_results_table_row_dac_snpv_high = []
        equity_results_table_row_li_ben = []
        equity_results_table_row_li_snpv_low = []
        equity_results_table_row_li_snpv_high  = []

        for ix, op in enumerate(self.host_report.chart_data, start=0):
            eo = op[1]
            # investment results
            investment_results_table_row_PV_power.append('{0:,.0f}'.format(eo.pv_capacity))
            investment_results_table_row_ESS_power.append('{0:,.0f}'.format(eo.power_capacity))
            investment_results_table_row_ESS_energy.append('{0:,.0f}'.format(eo.energy_capacity))
            pv_cost = eo.cost_per_MW_PV_system * eo.pv_capacity + eo.fixed_cost_of_PV_system
            ess_cost = eo.cost_per_MWh_BESS * eo.energy_capacity + eo.cost_per_MW_BESS * eo.power_capacity + eo.fixed_cost_of_the_BESS 
            total_cost = pv_cost + ess_cost
            investment_results_table_row_PV_cost.append(format_long_dollar_string(pv_cost))
            investment_results_table_row_ESS_cost.append(format_long_dollar_string(ess_cost))
            investment_results_table_row_total_cost.append(format_long_dollar_string(total_cost))
            
            # impact results
            impact_results_table_row_hb_low.append(format_long_dollar_string(eo.replacement_fraction*eo.Pollution_Low_Value))
            impact_results_table_row_hb_high.append(format_long_dollar_string(eo.replacement_fraction*eo.Pollution_High_Value))
            impact_results_table_row_co2_off.append('{0:,.0f}'.format(eo.CO2_emissions*eo.replacement_fraction))
            SCC = eo.CO2_emissions*eo.replacement_fraction*eo.cost_per_ton_of_CO2
            impact_results_table_row_climate_ben.append(format_long_dollar_string(SCC))
            NPV_multiplier = float(-npf.pv(eo.discount_rate,20,1,0))
            impact_results_table_row_snpv_health_low.append(format_long_dollar_string(NPV_multiplier*eo.replacement_fraction*eo.Pollution_Low_Value))
            impact_results_table_row_snpv_health_high.append(format_long_dollar_string(NPV_multiplier*eo.replacement_fraction*eo.Pollution_High_Value))
            impact_results_table_row_snpv_climate.append(format_long_dollar_string(NPV_multiplier*SCC))
            impact_results_table_row_snpv_total_low.append(format_long_dollar_string(NPV_multiplier*(SCC+eo.replacement_fraction*eo.Pollution_Low_Value)))
            impact_results_table_row_snpv_total_high.append(format_long_dollar_string(NPV_multiplier*(SCC+eo.replacement_fraction*eo.Pollution_High_Value)))

            # equity results
            equity_results_table_row_dac_pop.append('{0:,.1f}'.format(eo.disadvantaged_population_fraction*100))
            equity_results_table_row_li_pop.append('{0:,.1f}'.format(eo.low_income_population_fraction*100))
            equity_results_table_row_dac_ben.append('{0:,.1f}'.format(eo.impact_on_disadvantaged_population_fraction*100))
            low_dac_sNPV = eo.replacement_fraction*eo.Pollution_Low_Value*eo.impact_on_disadvantaged_population_fraction*NPV_multiplier
            high_dac_sNPV = eo.replacement_fraction*eo.Pollution_High_Value*eo.impact_on_disadvantaged_population_fraction*NPV_multiplier
            equity_results_table_row_dac_snpv_low.append(format_long_dollar_string(low_dac_sNPV))
            equity_results_table_row_dac_snpv_high.append(format_long_dollar_string(high_dac_sNPV))
            equity_results_table_row_li_ben.append('{0:,.1f}'.format(eo.impact_on_low_income_population_fraction*100))
            low_li_sNPV = eo.replacement_fraction*eo.Pollution_Low_Value*eo.impact_on_low_income_population_fraction*NPV_multiplier
            high_li_sNPV = eo.replacement_fraction*eo.Pollution_High_Value*eo.impact_on_low_income_population_fraction*NPV_multiplier
            equity_results_table_row_li_snpv_low.append(format_long_dollar_string(low_li_sNPV))
            equity_results_table_row_li_snpv_high.append(format_long_dollar_string(high_li_sNPV))

        investment_results_table.append(investment_results_table_row_PV_power)
        investment_results_table.append(investment_results_table_row_ESS_power)
        investment_results_table.append(investment_results_table_row_ESS_energy)
        investment_results_table.append(investment_results_table_row_PV_cost)
        investment_results_table.append(investment_results_table_row_ESS_cost)
        investment_results_table.append(investment_results_table_row_total_cost)

        impact_results_table.append(impact_results_table_row_hb_low)
        impact_results_table.append(impact_results_table_row_hb_high)
        impact_results_table.append(impact_results_table_row_co2_off)
        impact_results_table.append(impact_results_table_row_climate_ben)
        impact_results_table.append(impact_results_table_row_snpv_health_low)
        impact_results_table.append(impact_results_table_row_snpv_health_high)
        impact_results_table.append(impact_results_table_row_snpv_climate)
        impact_results_table.append(impact_results_table_row_snpv_total_low)
        impact_results_table.append(impact_results_table_row_snpv_total_high)

        equity_results_table.append(equity_results_table_row_dac_pop)
        equity_results_table.append(equity_results_table_row_li_pop)
        equity_results_table.append(equity_results_table_row_dac_ben)
        equity_results_table.append(equity_results_table_row_dac_snpv_low)
        equity_results_table.append(equity_results_table_row_dac_snpv_high)
        equity_results_table.append(equity_results_table_row_li_ben)
        equity_results_table.append(equity_results_table_row_li_snpv_low)
        equity_results_table.append(equity_results_table_row_li_snpv_high)

        investment_results_table_row = [] 
        impact_results_table_row = [] 
        equity_results_table_row = []
        for ix, op in enumerate(self.host_report.chart_data, start=0):
            eo = op[1]
            investment_results_table_row.append(eo.pv_capacity)
            impact_results_table_row.append(eo.Pollution_Low_Value)
            equity_results_table_row.append(eo.disadvantaged_population_fraction*100)

        investment_results_table.append(investment_results_table_row)
        impact_results_table.append(impact_results_table_row)
        equity_results_table.append(equity_results_table_row)
        

        # plot cumulative distrobution of dispatch before and after replacement 
        plt.figure(figsize=(6, 4), dpi=300 )
        for ix, op in enumerate(self.host_report.chart_data, start=0):
            impact_results_table_row = [] 
            eo = op[1]
            if ix == 0:
                plt.plot(range(eo.n),np.sort(eo.plant_dispatch),label="Baseline", linewidth=3) #
            ...
            replacement_fractions.append(eo.replacement_fraction*100)
        ...
        plt.savefig( distribution_chart_file, bbox_inches='tight', facecolor='white' )
        ...
        cost_benefit_chart_file = os.path.join(output_dir,'images', 'cost_benefit')
        plt.savefig( cost_benefit_chart_file, bbox_inches='tight', facecolor='white' )
        ...
        benefit_map_file = os.path.join(output_dir,'images', 'benefit_map')
        plt.savefig( benefit_map_file, facecolor='white' )

        ax.set_xlim(plant_dispatch_json["lon"]-3,plant_dispatch_json["lon"]+3)
        ax.set_ylim(plant_dispatch_json["lat"]-2,plant_dispatch_json["lat"]+2)
        benefit_map_file_close = os.path.join(output_dir,'images', 'benefit_map_close')
        plt.savefig( benefit_map_file_close, facecolor='white' )
        ...
        output = template.render(
                        ...
                        benefit_map_file_close=os.path.join('images', 'benefit_map_close.png'),
                        # TABLES
                        investment_results_table = investment_results_table,
                        impact_results_table = impact_results_table,
                        equity_results_table = equity_results_table
					)
        ...
        popup = WarningPopup()
        popup.title = 'Report Finished!'
        popup.popup_text.text  = 'The report has been completed. You can view it by clicking the Open Report button.'
        popup.open()
        self.open_report_button.disabled = False

    def open_report(self, *args):
