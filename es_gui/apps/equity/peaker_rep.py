...
class PeakerRepWizard(Screen):
    ...
    def on_enter(self):
        ...
        ab.set_title('Powerplant Replacement with ESS+PV')
...
class PeakerRepWizardPlantSelect(Screen):
    ...
    def _validate_inputs(self):
        if self.power_plant_selected == {}:
            raise InputError("Remember to select a powerplant from the list. If you don't see any options then return to the home screen and select Data Manager. Download powerplant data before returning to this wizard.")
...
class PeakerRepWizardSystemParameters(Screen):
    ...
    def _validate_inputs(self):
        ...
        if replacement_fractions == []:
            raise InputError("Select at least one replacement fraction")
    ...
    def open_param_help(self):
        """Opens the API key help ModalView."""
        open_param_view = ParametersHelpModalView()
        open_param_view.open()
    
    def _next_screen(self):
        if not self.manager.has_screen('model_assumption'):
            screen = PeakerRepWizardModelAssumption(name='model_assumption')
            self.manager.add_widget(screen)
        try:
            self.get_selections()
        except InputError as e:
            ...
            self.manager.transition.duration = BASE_TRANSITION_DUR
            self.manager.transition.direction = 'left'
            self.manager.current = 'model_assumption'

class ParametersHelpModalView(ModalView):
    """ModalView to display instructions on how to get a Data.gov (NREL Developer Network) API key."""

class PeakerRepWizardModelAssumption(Screen):
    ...
    def _flexible_button_on_release(self):
        self.fixed_button.state = 'normal'

    def _fixed_button_on_release(self):
        self.flexible_button.state = 'normal'

    def _validate_inputs(self):
        if self.flexible_button.state == 'down':
            flexible_dispatch = True
            fixed_dispatch = False
        if self.fixed_button.state == 'down':
            flexible_dispatch = False
            fixed_dispatch = True

        params = {}
        params['flexible_dispatch'] = flexible_dispatch
        params['fixed_dispatch'] = fixed_dispatch

        params_desc = []
        flexible_dispatch_desc = 'Flexible_dispatch : ' + str(flexible_dispatch)
        params_desc.append(flexible_dispatch_desc)

        fixed_dispatch_desc ='Fixed_dispatch : ' + str(fixed_dispatch) 
        params_desc.append(fixed_dispatch_desc)
...
class PeakerRepWizardSummary(Screen):
    ...
    def get_selections(self):
        ...
        op_handler_requests['dispatch_model'], op_handler_requests['dispatch model desc'] = sm.get_screen('model_assumption').get_selections()
    ...
    def on_enter(self):
        ...
        model_param = op_handler_requests['dispatch model desc']
