...
class EquityOptimizerHandler:# create a spesific function for peaker palnt replacement 
    ...
    def process_requests(self, op_handler_requests, *args):
        ...
        dispatch_model = op_handler_requests['dispatch_model']
        ...
        for rp in replacement_fractions:
            ...
            with open(plant_data_path['path']) as f:
                ...
                # COBRA data calculated by adding tier 1 emmissions so benifits are negative impacts
            ...
            op.flexible_dispatch = dispatch_model['flexible_dispatch']
            op.fixed_dispatch = dispatch_model['fixed_dispatch']
            ...
            # now we sort the profile to determin the dispatch percentials
            total_energy = sum(plant_dispatch)
            sort_plant_dispatch = np.sort(plant_dispatch)
            cum_sum_energy = 0
            for i in range(op.n):
                cum_sum_energy = cum_sum_energy + sort_plant_dispatch[i]
                if cum_sum_energy >= total_energy*rp:
                    energy_percentile_power = sort_plant_dispatch[i]
                    break

            filtered_plant_dispatch = [0 for i in range(op.n)]
            for i in range(op.n):
                if plant_dispatch[i] <= energy_percentile_power:
                    filtered_plant_dispatch[i] = plant_dispatch[i]


            op.plant_dispatch = [plant_dispatch[i] for i in op.time]
            op.filtered_plant_dispatch = [filtered_plant_dispatch[i] for i in op.time]
    ...
    def _save_to_solved_ops(op,rp):
        ...
        if op.flexible_dispatch:
            name_components.append(' | flexible dispatch')
        if op.fixed_dispatch:
            name_components.append(' | fixed dispatch')
