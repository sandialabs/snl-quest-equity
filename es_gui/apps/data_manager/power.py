...
class PowerPlantSearchScreen(Screen):
    ...
    def on_enter(self):
        ...
        def _go_to_webpage(instance, value):
            if value == 'fact-api':
                ...
                webbrowser.open('https://screeningtool.geoplatform.gov/en/#3/33.47/-97.5')
        ...
        text4 = 'Last, it multiplies the health impacts by the demography from the {Justice40} in each county to determine the distributional equitably.'.format(
            Justice40=_ref_link('Climate and Economic Justice Screening Tool', 'justice40'))
    ...
    def _query_api(self, power_plant_params):
        ...
        if not FOUND:
            self.save_button.disabled = False
            popup = WarningPopup()
            popup.popup_text.text = 'ORIS ID not found in Facilities List.'
            popup.open()
            return
        else:
            outname = self.save_name_field.text
        ...
        try:
            ...
            if row_number == 0:
                ...
                # Plant state
                request_content['State'] = Plants_spreadsheet.cell(
                    row=row_number, column=4).value
                # Plant county 
                request_content['County'] = Plants_spreadsheet.cell(
                    row=row_number, column=20).value
                ...
                request_content['PPNC_net_generation'] = float(
                    Plants_spreadsheet.cell(row=row_number, column=41).value)
